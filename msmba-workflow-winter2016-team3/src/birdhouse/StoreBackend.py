'''
Back end for the store bar workflow (Winter Intensives Lab 5)
...
class StoreBackend(Backend):
    ...
    def drink_order_taken(self, results):
        ...
        for result in results:  # repeat the following actions for each result
            # !!! Fix the line below...
            task = Task.construct_from_result(result,"Warehouse","Processing") 
    ...
    def drink_prepared(self, results):
        ...
        for result in results:  # repeat the following actions for each result
            # !!! Fix the line below...
            self.workflow.update_status(result, Status.SOLD)
